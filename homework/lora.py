...
class LoRALinear(HalfLinear):
    ...
    def __init__(
        ...
        self.weight.data.requires_grad_(False)
        ...
        # initialize the low rank adapters
        torch.nn.init.kaiming_uniform_(self.lora_a.weight)
        torch.nn.init.zeros_(self.lora_b.weight)
        self.lora_a.weight.data.requires_grad_(True)
        self.lora_b.weight.data.requires_grad_(True)


    def forward(self, x: torch.Tensor) -> torch.Tensor:
        base = super().forward(x)  # float32
